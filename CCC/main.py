...
def main_work(args):
    ...
    if args.warmup_with_dbscan:
        warmup_ckpt_dir = '[DBSCAN_warmup]'
    else:
        warmup_ckpt_dir = '[HDC' if args.hdc_outlier else '[HDC#'
        warmup_ckpt_dir += f'({args.num_clusters})_warmup]'
    ...
    # Cluster
    warmup_cluster = DBSCAN(eps=args.dbscan_eps, metric='precomputed',
                            min_samples=args.cluster_min_instances, n_jobs=args.num_workers)
    HDC = __HDC_factory[args.hdc_camera_version]
    cluster = HDC(args.num_clusters,
                  max_iter=args.hdc_max_iter,
                  init=args.hdc_init,
                  normalize_centroid=args.hdc_normalize,
                  similarity=args.hdc_similarity,
    ...
    for i_epoch in range(args.num_epochs):
        if i_epoch < args.warmup_iters and load_warmup_ckpt and exist_warmup_ckpt:
            continue
        # Exact features
        features, camera_indexes = exact_features(model, cluster_loader, camera=True)

        # Update centers & pseudo labels
        print('Updating centers & pseudo labels...', end='')
        start = time.time()
        if i_epoch < args.warmup_iters and args.warmup_with_dbscan:
            ...
            memory.pseudo_labels = warmup_cluster.fit_predict(rerank_dist)
            ...
            if not args.hdc_camera_version:
                camera_indexes = None
            if not args.no_jaccard:
                rerank_dist = compute_jaccard_distance(features, k1=args.jaccard_k1, k2=args.jaccard_k2)
                print('(Jaccard distance using {:.5}s.)'.format(time.time() - start), end=' ')
                memory.update_labels(features, args.hdc_outlier, rerank_dist,
                                     camera_indexes=camera_indexes, min_instances=args.cluster_min_instances)
            else:
                memory.update_labels(features, args.hdc_outlier,
                                     camera_indexes=camera_indexes, min_instances=args.cluster_min_instances)
            if not args.hdc_centroids:
                centroids = generate_cluster_features(memory.pseudo_labels, features)
                memory.cluster_memory.features = F.normalize(centroids, dim=1).to(device)
        num_outliers = 0
        new_dataset = []
