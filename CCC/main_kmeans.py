...
def main_work(args):
    ...
    warmup_ckpt_dir = '[DBSCAN_warmup]' if args.warmup_with_dbscan else f'[KMeans({args.num_clusters})_warmup]'
    ...
    cluster = KMeans(n_clusters=args.num_clusters, init='k-means++', n_init='auto')
    ...
    for i_epoch in range(args.num_epochs):
        ...
        if i_epoch < args.warmup_iters and args.warmup_with_dbscan:
            ...
            memory.update_labels(features, kmeans=True, min_instances=args.cluster_min_instances)
        num_outliers = 0
        new_dataset = []
        ...
        if (i_epoch+1) >= args.eval_start_epoch and ((i_epoch+1) % args.eval_step_epoch == 0 or i_epoch == 0):
            ...
            if args.eval_and_ckpt and i_epoch > 0:
                ...
                torch.save(state_dict, osp.join(warmup_ckpt_dir, f'model_e{i_epoch+1}.pth'))
            # Save best_mAP model
            if new_best:
                state_dict = collections.OrderedDict()
                for k, v in model.state_dict().items():
                    state_dict[k.strip('module.')] = copy.deepcopy(v).cpu()
                torch.save(state_dict, osp.join(warmup_ckpt_dir, f'model_best.pth'))
        else:
...
def main():
    args = config(kmeans=True).parse_args()
    if args.no_jaccard:
        del args.jaccard_k1
        del args.jaccard_k2
    del args.hdc_outlier
    if 'ibn' in args.arch:
